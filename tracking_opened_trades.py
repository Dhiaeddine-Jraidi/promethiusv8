...
def process_trade(trade_id, trade_data, trade_to_remove, open_trades):
    ...
    if trade_result is not None:
        ...
        gb_classifier = int(trade_data['gradientboostingclassifier'] * 100) if not pd.isna(trade_data['gradientboostingclassifier']) else ''
        rf_classifier = int(trade_data['randomforestclassifier'] * 100) if not pd.isna(trade_data['randomforestclassifier']) else ''
        xgb_classifier = int(trade_data['xgbclassifier'] * 100) if not pd.isna(trade_data['xgbclassifier']) else ''
...
def process_finished_trade(trade_data, trade_to_remove1):
    ...
    if period_hours >= 37:
        ...
        trade_data = trade_data.drop(['gradientboostingclassifier', 'randomforestclassifier', 'xgbclassifier'])
