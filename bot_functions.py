...
def routine(strategy, timeframe, symbol, take_profit_percent, stop_loss_percent, side, calibration_perc):
    ...
    model_directory = "files/ml/"
    model_files = [file for file in os.listdir(model_directory) if file.endswith(".pkl")]
    probability_dict = {}
    for model_file in model_files:
        model_path = os.path.join(model_directory, model_file)
        model_name = os.path.splitext(model_file)[0]
        probability_dict[model_name] = predict_probability(data_dict, model_path)
    
    trade_data.append(probability_dict)
