...
import csv
import json
from ml_predict import predict_probability

JSON_FILE_PATH = "files/last_ml_train_check_time.json"
BOT_TOKEN_key = "6566859554:AAHN0ZeuI4Ojrd5II60lcH6y8-hIJ4OLXMk"
open_trade_csv = "files/download/open_trades.csv"
open_trade_columns = ['trade_id', 'symbol', 'entry_time_str', 'side', 'entry_price', 'last_price','take_profit_percent', 'stop_loss_percent', 'exit_price_TP', 'exit_price_SL','timeframe', 'strategy', 'pnl', 'period_hours', 'entry_day_of_week' , 'entry_hour', 'aroon_up', 'aroon_down','ema_short', 'ema_middle', 'ema_long', 'rsi', 'macd', '24h_volume']
...
def routine(strategy, timeframe, symbol, take_profit_percent, stop_loss_percent, side, calibration_perc):
    ...
    for model_file in model_files:
        ...
        probabilities_dict[model_name] = predict_probability(data_dict, model_path, number_of_previous_trades=20)
    
    open_trade_columns.extend(map(str.lower, probabilities_dict.keys()))
    ...
    for model_name, probability in probabilities_dict.items():
        if probability == '':
            probabilities_message += f"{model_name}:\n"     
        else:
            probabilities_message += f"{model_name}: {int(probability*100)}%\n"
...
def save_last_ml_train_check_time(last_ml_train_check_time):
    with open(JSON_FILE_PATH, 'w') as file:
        json.dump(last_ml_train_check_time, file)

def load_last_ml_train_check_time():
    try:
