...
from sql_integration import extract_trade_from_sql, table_name1, table_name2
...
def main_bot():
    ...
    ######################### train models each 72 hours.
    
    last_ml_train_check_time = load_last_ml_train_check_time()
    current_time = time.time()
    
    if current_time - last_ml_train_check_time >= 72 * 3600:
        train_models()
        save_last_ml_train_check_time(current_time)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(output_final_trades):
        try:
            df = extract_trade_from_sql(table_name1)
            df.to_csv(output_final_trades, index = False)
        except Exception as e:
            pass

    if not os.path.exists(output_final_trades2):
        try:
            df = extract_trade_from_sql(table_name2)
            df.to_csv(output_final_trades2, index = False)
        except Exception as e:
            pass
