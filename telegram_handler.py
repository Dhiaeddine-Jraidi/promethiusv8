...
async def soft_update_script(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        process = await asyncio.create_subprocess_exec('bash', './files/sh/soft_update_script.sh', stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
        _, stderr = await process.communicate()
        
        if stderr and b"error" in stderr.lower():
            with open("./files/logger/bash_logger.txt", "a") as file:
...
async def hard_update_script(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        process = await asyncio.create_subprocess_exec('bash', './files/sh/hard_update_script.sh', stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
        _, stderr = await process.communicate()
...
async def get_status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        process = await asyncio.create_subprocess_exec('sudo', 'supervisorctl', '-c', '/etc/supervisor/supervisord.conf', 'status', stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        stdout, stderr = await process.communicate()
        
        if stderr and b"error" in stderr.lower():
            await update.message.reply_text(f"Error executing command: {stderr.decode()}")
            with open("./files/logger/bash_logger.txt", "a") as file:
                file.write(stderr.decode())     
        else:
...
async def stop_bot(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        process = await asyncio.create_subprocess_exec('sudo', 'supervisorctl', '-c', '/etc/supervisor/supervisord.conf', 'stop', 'main', 'tracking_opened_trades', stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        stdout, stderr = await process.communicate()
...
async def start_bot(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        process = await asyncio.create_subprocess_exec('sudo', 'supervisorctl', '-c', '/etc/supervisor/supervisord.conf', 'start', 'main', 'tracking_opened_trades', stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        _, stderr = await process.communicate()
